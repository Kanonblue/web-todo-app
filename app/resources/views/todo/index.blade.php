...
@section('content')
    <div class="container">
        ...
        <div class="text-end">
             @php
        $isDone = request()->has('done');  // 完了リストかどうか
    @endphp
            <div class="text-end">
    <a href="{{ route('todo.index', ['done' => true]) }}"
       class="pe-3 text-decoration-none text-black
              {{ $isDone ? 'fw-bold text-decoration-underline' : '' }}">
        完了リスト
    </a>
    |
    <a href="{{ route('todo.index') }}"
       class="ps-3 pe-3 text-decoration-none text-black
              {{ !$isDone ? 'fw-bold text-decoration-underline' : '' }}">
        未完了リスト
    </a>
            ...
            @foreach($todos as $todo)
                <div class="mx-auto">
                    <div class="card mb-3">
                        ...
                        <div class="card-body text-start task-title">
                            {{ $todo->title }}
                        </div>
